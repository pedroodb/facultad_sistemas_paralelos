...
void nextComb(int skip, int tomadosDe, int cantElems, int* comb, int id){
    ...
    while (carry > 0 && (comb[0] <= (cantElems-tomadosDe))){
        ...
        //printf("indexOver:%d  comb[iO]:%d",indexOver,comb[indexOver]);
...
int buscarEnLista(int cantElems, int tomadosDe, int suma, int skip, int* comb, int* lista, int id){
	...
	while(comb[0] <= (cantElems-tomadosDe)){
        ...
        nextComb(skip,tomadosDe,cantElems,comb,id);
...
int slave(int id, int cantProc, int* lista, int cantElems, int tomadosDe, int suma){
    ...
    nextComb(id,tomadosDe,cantElems,comb,id);
    int cant = buscarEnLista(cantElems,tomadosDe,suma,cantProc,comb,lista,id);
	free(comb);
    return cant;
}
